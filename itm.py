...
from model import UniterForImageTextRetrieval, VLXLMRForImageTextRetrieval
from optim import get_lr_sched, get_xlmr_lr_sched
from optim.misc import build_optimizer, build_xlmr_optimizer
...
def rename_checkpoint(checkpoint, add_prefix="roberta."):
    old_keys = []
    new_keys = []
...
def main(opts):
    ...
    for txt_path, img_path in zip(opts.train_txt_dbs, opts.train_img_dbs):
        if "itm_coco_zh" not in txt_path:
            ...
            if opts.hard_neg_size > 0:
                ...
                train_datasets.append(ItmRankDataset_COCO_CN(txt_db, img_train_db, img_val_db,
                                                     opts.negative_size))
    ...
    for test_img_db_path, test_txt_db_path in zip(opts.test_img_db, opts.test_txt_db):
        if "itm_coco_zh" not in test_txt_db_path:
            ...
            test_txt_db = TxtTokLmdb(test_txt_db_path, -1)
            eval_dataset_test = ItmEvalDataset_COCO_CN(test_txt_db, test_img_train_db, test_img_val_db, opts.inf_minibatch_size)
    ...
    model = VLXLMRForImageTextRetrieval.from_pretrained(
            opts.model_config, state_dict=checkpoint,load_embedding_only=opts.load_embedding_only,load_layer=opts.load_layer,
    ...
    while True:
        train_dataloader = build_dataloader(
            train_dataset, xlmr_itm_rank_collate, True, opts)
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % opts.gradient_accumulation_steps == 0:
                ...
                if opts.separate_lr:
                    ...
                    TB_LOGGER.add_scalar('xlmr_lr', xlmr_lr_this_step, global_step)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--xlmr_lr",
                        default=1e-5,
                        type=float,
                        help="The initial learning rate for xlmr text embedding")
    ...
    parser.add_argument('--pin_mem', action='store_true',
                        help="pin memory")
    parser.add_argument('--rename_checkpoints', action='store_true',
                        help="whether to rename the keys in the checkpoints")
    parser.add_argument('--separate_lr', action='store_true',
                        help="seprate lr for different modules in the model")
    parser.add_argument('--load_embedding_only', action='store_true',
                        help="only load the embedding layer from the XLMR model")
    parser.add_argument('--load_layer', type=int, default=0,
                        help="number of layers to load from XLMR")
    parser.add_argument("--save_steps", default=200, type=int,
                        help="save every X steps for "
                             "continue after preemption")
    parser.add_argument("--mscoco_cn", action='store_true', help='check whether it is finetuned on mscoco')
